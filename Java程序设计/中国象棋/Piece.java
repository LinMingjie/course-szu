...
public class Piece {
    ...
    public static Vector<Destination> possibleDestinations(char board[][], SquareCoord coord, boolean checkCheck) {
    ...
    protected void checkMove(Vector<Destination> moves, int x, int y, char piece, char color, char[][] tempBoard, char[][] board, boolean checkCheck) {
        ...
        if ((!checkCheck) || (!Game.checkForCheck(tempBoard, color))) {
            if (board[x][y] == ' ') {
                moves.add(new Destination(x, y, false));
            } else {
                switch (color) {
                    case 'r':
                        if (Character.isLowerCase(board[x][y])) {
                            moves.add(new Destination(x, y, true));
                        }
                        break;
                    case 'b':
