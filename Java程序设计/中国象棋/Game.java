...
public class Game {
    ...
    public Stack backCapture = new Stack();
    public Stack frontCapture = new Stack();
     ...
     *
     * @para position - FEN like string of characters representing the state of
     * a game.
    ...
    private void setTurn(char color) throws InvalidPositionException {
        if (color == BLUE || color == RED) {
            ...
            throw new InvalidPositionException("Unknown color: " + color);
        }
    }
    ...
    public void reNotifyObservers(String move,char type) {
        for (Enumeration e = observers.elements(); e.hasMoreElements();) {
            ((GameObserver) e.nextElement()).reMove(move,type);
        }
    }
    ...
    public Vector possibleMoves(SquareCoord coord, boolean checkCheck) {
        ...
        switch (pieceAtCoord(coord)) {
            case 'K':
            case 'k':
                return Jiang.possibleDestinations(board, coord, checkCheck);
            case 'A':
            case 'a':
                return Shi.possibleDestinations(board, coord, checkCheck);
            case 'H':
            case 'h':
                return Ma.possibleDestinations(board, coord, checkCheck);
            case 'C':
            case 'c':
                return Pao.possibleDestinations(board, coord, checkCheck);
            case 'R':
            case 'r':
                return Ju.possibleDestinations(board, coord, checkCheck);
            case 'E':
            case 'e':
                return Xiang.possibleDestinations(board, coord, checkCheck);
            case 'P':
            case 'p':
                return Zu.possibleDestinations(board, coord, checkCheck);
            default:
                return new Vector();
        }
    }
     ...
     *
     * @param moveText text of move - format is d3-d4 where d3 is the origin and
     * d4 is the destination
     */
    public boolean move(String moveText) {
        ...
        if (origin.equals(destin)) {
            return false;
        } // don't mess with non-moves.
        boolean legalMove = false; // assume it is not...
        char piece = pieceAtCoord(origin);
        if ((toMove == RED && !Character.isUpperCase(piece))) {
            return false;
        }
        if ((toMove == BLUE && Character.isUpperCase(piece))) {
            return false;
        }
      ...
      public void forward(){
        if(!front.isEmpty()){
            ...
            String moveText=front.pop();
            SquareCoord origin = new SquareCoord(moveText.substring(0, moveText.indexOf('-')));
            SquareCoord destin = new SquareCoord(moveText.substring(moveText.indexOf('-') + 1));
            char origPiece = pieceAtCoord(origin);
            char destPiece = pieceAtCoord(destin);
            if (board[destin.getRow()][destin.getFile()] != ' ') {
                captures += board[destin.getRow()][destin.getFile()];
                backCapture.push(1);
            }
            ...
            board[origin.getRow()][origin.getFile()] = ' ';
            board[destin.getRow()][destin.getFile()] = origPiece;
            toMove ^= TURNSWITCH;
            reNotifyObservers(moveText,'F');
        }
//        System.out.println();
//        printBoard();
    }
    ...
    public void backward(){
        ...
        if(!back.isEmpty()){
            ...
            String moveText=back.pop();
            SquareCoord origin = new SquareCoord(moveText.substring(moveText.indexOf('-') + 1));
            SquareCoord destin = new SquareCoord(moveText.substring(0, moveText.indexOf('-')));
            char origPiece = pieceAtCoord(origin);
            char destPiece = pieceAtCoord(destin);
            flag=false;
            if(!backCapture.isEmpty() && backCapture.peek()==(Integer)1){
                destPiece=captures.charAt(captures.length()-1);
            }
            board[origin.getRow()][origin.getFile()] = destPiece;
            board[destin.getRow()][destin.getFile()] = origPiece;
            toMove ^= TURNSWITCH;
            moveText=moveText.substring(moveText.indexOf('-') + 1) + '-'+moveText.substring(0, moveText.indexOf('-'));
            reNotifyObservers(moveText,'B');
        //    checkForMate();
//            printBoard();
        }
    }
            ...
            for (int j = 0; j < BOARD_WIDTH && mate == MATE_STALE; j++) {
                ...
                if ((toMove == RED && Character.isUpperCase(board[i][j]))
                    ...
                    switch (board[i][j]) {
                        ...
                        case 'R':
                        case 'r':
                            v = Ju.possibleDestinations(board, coord, checkCheck);
                            break;
                        case 'E':
                        case 'e':
                ...
                if (board[i][j] != ' ') {
                    if ((color == 'r' && Character.isLowerCase(board[i][j]))
                        ...
                        switch (board[i][j]) {
                            ...
                            case 'R':
                            case 'r':
                                possible = Ju.possibleDestinations(board, new SquareCoord(i, j), false);
                                break;
                            case 'H':
                            case 'h':
