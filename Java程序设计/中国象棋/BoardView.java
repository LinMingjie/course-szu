...
class BoardView extends JPanel implements GameObserver {
    ...
    public void reMove(String move,char type) {
        if(type=='B'){
            SquareCoord origin = new SquareCoord(move.substring(0, move.indexOf('-')));
            SquareCoord destin = new SquareCoord(move.substring(move.indexOf('-') + 1));
            char destPiece = board[destin.getRow()][destin.getFile()].getPiece();
            boolean flag=false;
            if(!game.backCapture.isEmpty() && game.backCapture.pop()==(Integer)1){
                    destPiece=game.captures.charAt(game.captures.length()-1);
                    game.captures=game.captures.substring(0, game.captures.length()-1);
                    flag=true;
            }
            clearHighlights();
            if (reverse) {
            ...
            if (flag) {
                Image im = (Image) pieceImages.get((Object) ((new String("")) + destPiece));
                board[9 - origin.getRow()][8 - origin.getFile()].setPieceImage(im);
                board[9 - origin.getRow()][8 - origin.getFile()].setPiece(destPiece);
            }
        }
        else{
            SquareCoord origin = new SquareCoord(move.substring(0, move.indexOf('-')));
